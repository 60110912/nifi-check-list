...
def nifi_api_usage(config):
    try:
        ...
        # token = nipyapi.client.create_access_token(username=config['credentials']['login'], password=config['credentials']['password'])
...
def main():
    ...
    try:
        ...
        print("Передаю конфигарацию")
        print("Config")
        print(config)
        config['credentials']['password'] = decryptSecret(config['credentials']['encrypPassword'])
        ...
        print("Запускаю тестовый кейс") 
        nifi_api_usage(config)
        print("Тестовый кейс прошел") 
    except Exception as e:
